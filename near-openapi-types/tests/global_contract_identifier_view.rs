//! Test to reproduce GlobalContractIdentifierView deserialization issue
//!
//! The RPC returns DeterministicStateInit action with `code` as a bare string:
//! ```json
//! {"DeterministicStateInit":{"code":"mt_receiver_global.sandbox","data":{},"deposit":"0"}}
//! ```
//!
//! But GlobalContractIdentifierView uses #[serde(flatten)] which doesn't handle bare strings.

use near_openapi_types::{ActionView, GlobalContractIdentifierView};
...
fn test_global_contract_identifier_view_from_string() {
    ...
    let result: Result<GlobalContractIdentifierView, _> = serde_json::from_str(json);
...
fn test_deterministic_state_init_action_view() {
    ...
    let result: Result<ActionView, _> = serde_json::from_str(json);
